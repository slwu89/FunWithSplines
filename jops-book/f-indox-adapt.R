# Adaptive smoothing with SOP (indiumoxide data)
...
fit <- adaptive1D.SOP(x = x, y = y, smooth.cov = fitpar, adaptive = T,
...
plt1 = ggplot(Pf, aes(x = x, y = y) ) +
       ...
       JOPS_theme() +
       ggtitle('X-ray diffraction of indium oxide with adaptive smooth') +
