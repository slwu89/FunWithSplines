# Box-Cox-Cole-Green smoothing of weights of boys (Dutch boys data)
...
# Fit Box-Cox-Cole-Green model with GAMLSS
g = gamlss(y ~ pb(x), sigma.formula = ~ pb(x), nu.formula = ~ pb(x),
           data = Data, family = BCCG)
...
plt1 = ggplot(Data, aes(x = x, y = y)) +
  ...
  ggtitle("GAMLSS (BCCG) fit for Dutch boys") +
